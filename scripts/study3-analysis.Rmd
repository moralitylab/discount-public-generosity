---
title: "JTF 13 + 14: Study 3 Analysis"
author: "Nathan Liang"
date: "May 8, 2023"
---
...
study3_df <- readr::read_csv(here("study-13-14/data/clean/study3-clean.csv"),
  show_col_types = F
)
```
...
set.seed(7)
ggplot(
  study3_plot_df, aes(x = vignette, y = moral_goodness, fill = condition)
) +
...
ggsave(here("study-13-14/figures/study3-moral-goodness-violin.png"),
  width = 20, height = 5
)
```
...
set.seed(7)
ggplot(
  study3_plot_df, aes(x = vignette, y = trust, fill = condition)
) +
  geom_point(aes(fill = condition, color = condition),
    alpha = 0.5, stroke = 0, size = 1,
    position = position_jitterdodge(jitter.height = 0.25, 0.75)
...
ggsave(here("study-13-14/figures/study3-trust-violin.png"),
  width = 20, height = 5
)
```
...
```{r plot-principled-motivation-violin}
set.seed(7)
ggplot(
  study3_plot_df, aes(x = vignette, y = principled_motivation, fill = condition)
) +
  geom_point(aes(fill = condition, color = condition),
    alpha = 0.5, stroke = 0, size = 1,
    position = position_jitterdodge(jitter.height = 0.25, 0.75)
...
ggsave(here("study-13-14/figures/study3-principled-motivation-violin.png"),
  width = 20, height = 5
)
```
...
```{r plot-reputation-signaling-violin}
set.seed(7)
ggplot(
  study3_plot_df, aes(x = vignette, y = rep_signal, fill = condition)
) +
  geom_point(aes(fill = condition, color = condition),
    alpha = 0.5, stroke = 0, size = 1,
    position = position_jitterdodge(jitter.height = 0.25, 0.75)
...
ggsave(here("study-13-14/figures/study3-reputation-signaling-violin.png"),
  width = 20, height = 5
)
```
...
```{r plot-normative-expectation-violin}
set.seed(7)
ggplot(
  study3_plot_df, aes(x = vignette, y = norm_expect, fill = condition)
) +
  geom_point(aes(fill = condition, color = condition),
    alpha = 0.5, stroke = 0, size = 1,
    position = position_jitterdodge(jitter.height = 0.25, 0.75)
...
ggsave(here("study-13-14/figures/study3-normative-expectation-violin.png"),
  width = 20, height = 5
)
```
...
study3_df %>%
  ...
  dplyr::summarize(
    ...
    moral_goodness_mean = format(
      round(mean(moral_goodness, na.rm = T), 2),
      nsmall = 2
    ),
    moral_goodness_sd = format(
      round(sd(moral_goodness, na.rm = T), 2),
      nsmall = 2
    ),

    # Trust
    trust_mean = format(
      round(mean(trust, na.rm = T), 2),
      nsmall = 2
    ),
    trust_sd = format(
      round(sd(trust, na.rm = T), 2),
      nsmall = 2
    ),

    # Principled Motivation (Norm Signaling)
    principled_motivation_mean = format(
      round(mean(principled_motivation, na.rm = T), 2),
      nsmall = 2
    ),
    principled_motivation_sd = format(
      round(sd(principled_motivation, na.rm = T), 2),
      nsmall = 2
    ),

    # Reputation Signaling
    rep_signal_mean = format(
      round(mean(rep_signal, na.rm = T), 2),
      nsmall = 2
    ),
    rep_signal_sd = format(
      round(sd(rep_signal, na.rm = T), 2),
      nsmall = 2
    ),

    # Normative Expectation
    norm_expect_mean = format(
      round(mean(norm_expect, na.rm = T), 2),
      nsmall = 2
    ),
    norm_expect_sd = format(
      round(sd(norm_expect, na.rm = T), 2),
      nsmall = 2
    )
  ) %>%
  dplyr::ungroup() %>%
  tidyr::pivot_wider(
    ...
    values_from = c(
      ...
      trust_mean,
      trust_sd,
      principled_motivation_mean,
      principled_motivation_sd,
      rep_signal_mean,
      rep_signal_sd,
      norm_expect_mean,
      norm_expect_sd
  ...
  readr::write_csv(here(
    "study-13-14/data/tables/study3-descriptive-statistics-table.csv"
  ))
```
